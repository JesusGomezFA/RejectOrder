        ...
        public void Send(string fileName)
        {
            ...
            using (SqlConnection conect = new SqlConnection(GetConnectionSql()))
            {
                ...
                catch (Exception)
                {
                    ...
                    string insert = "insert into MM_Log(Fecha,Problema,Consola) values ('" + FechaArchivo + "_" + horaArchivo + "','Error en las credenciales sftp','RejectedOrders')";
                ...
                using (var sftp = new SftpClient(connectionInfo))
                {
                    ...
                    sftp.Connect();
                    sftp.ChangeDirectory(@"/MoneyAlianza/RejectedOrders");
                    using (var uplfileStream = System.IO.File.OpenRead(fileName))
                    {
                        sftp.UploadFile(uplfileStream, "RejectedOrders_" + FechaArchivo + "_" + horaArchivo + ".csv", true);
                    }
                    sftp.Disconnect();
                }
            }
        ...
        public string GetConnectionSql()
        {
            ...
            catch (Exception ex)
            {
                ...
                string insert = $"insert into MM_Log(Fecha,Problema,Consola) " +
                    $"values ('{FechaArchivo()}','problema al conectar con base de datos SQL','RejectedOrders')";
